...
class GreedRegulator(QWidget):
    ...
    def __init__(self, presentation: RegulatorGuiPresentation | None = None) -> None:
        ...
        # ─────── SIMULATE BUTTON ─────────────────────────
        self.sim_btn = QPushButton("Simulate Greed Update — disabled")
        self.sim_btn.clicked.connect(self._simulate_update)
        scroll_layout.addWidget(self.sim_btn)

        # ─────── CLEAR BUTTON ─────────────────────────────
        self.clr_btn = QPushButton("Clear Log — disabled")
        self.clr_btn.clicked.connect(self._clear_log)
        scroll_layout.addWidget(self.clr_btn)
        ...
        self.display_analysis(presentation or RegulatorGuiAdapter.unavailable("greed", "No analyzer evidence loaded."))

    def _disable_legacy_controls(self) -> None:
        for control in (*self.weight_sliders.values(), self.threshold, self.sim_btn, self.clr_btn, self.save_btn):
    ...
    def display_analysis(self, presentation: RegulatorGuiPresentation) -> None:
        """Display already-produced GREED_VECTOR_V1 evidence without recalculating it."""
        if presentation.regulator != "greed":
            raise ValueError("GreedRegulator accepts only greed presentation evidence")
        score = presentation.overall_score or 0.0
        self.greed_value = score
        self.greed_bar.set_progress(score / 100.0)
        suffix = "" if presentation.available else " (unavailable)"
        self.greed_label.setText(f"Greed: {score:g} / 100{suffix}")
        for key, bar in self.sub_widgets.items():
            value = presentation.subscores.get(key, 0.0)
            self.subscores[key] = value
            bar.set_progress(value / 100.0)
