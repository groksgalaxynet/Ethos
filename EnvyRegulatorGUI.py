...
class EnvyRegulator(QWidget):
    def __init__(self, presentation: RegulatorGuiPresentation | None = None) -> None:
        ...
        # ----------------- SUBCHANNEL LIST ------------------------
        sub_title_label = QLabel("Current Analyzer Channel + Legacy Channels")
        ...
        for key in self.subscores:
            lbl = QLabel(key if key == "SOCIAL_COMPARISON" else f"{key} (legacy unavailable)")
            bar = BarWidget()
        ...
        self.btn_update = QPushButton("Simulate Envy Update — disabled")
        self.btn_update.clicked.connect(self._simulate_update)
        ...
        # New: Save State
        self.btn_save = QPushButton("Save State — disabled")
        self.btn_save.clicked.connect(self._save_state)
        button_layout.addWidget(self.btn_save)

        main_layout.addLayout(button_layout)
        self._disable_legacy_controls()
        self.display_analysis(presentation or RegulatorGuiAdapter.unavailable("envy", "No analyzer evidence loaded."))

    def _disable_legacy_controls(self) -> None:
        for control in (self.threshold_slider, self.btn_update, self.btn_clear, self.btn_save):
    ...
    def display_analysis(self, presentation: RegulatorGuiPresentation) -> None:
        """Display already-produced ENVY_VECTOR_V1 evidence without recalculating it."""
        if presentation.regulator != "envy":
            raise ValueError("EnvyRegulator accepts only envy presentation evidence")
        score = presentation.overall_score or 0.0
        self.envy_value = score
        self.envy_bar.set_progress(score / 100.0)
        suffix = "" if presentation.available else " (unavailable)"
        self.envy_label.setText(f"Envy: {score:g} / 100{suffix}")
        self.subscores["SOCIAL_COMPARISON"] = presentation.subscores.get("SOCIAL_COMPARISON", 0.0)
        self.sub_widgets["SOCIAL_COMPARISON"].set_progress(self.subscores["SOCIAL_COMPARISON"] / 100.0)
